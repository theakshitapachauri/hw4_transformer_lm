...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        q = self.q(x).view(bs, seq, self.n_heads, _ //self.n_heads).transpose(1, 2)
        k = self.k(x).view(bs, seq, self.n_heads, _ //self.n_heads).transpose(1, 2)
        v = self.v(x).view(bs, seq, self.n_heads, _ //self.n_heads).transpose(1, 2)
